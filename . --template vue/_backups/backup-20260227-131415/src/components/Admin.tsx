...
const inputBaseStyle: React.CSSProperties = {
  width: '100%', padding: '8px 10px', borderRadius: 12, border: '1px solid rgba(255,255,255,.12)', backgroundColor: 'rgba(255,255,255,.06)', color: '#fff',
}
    ...
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
        ...
        <tbody>{rows.map((row) => <tr key={row.id} style={{ borderTop: '1px solid #1f2937' }}>{columns.map((c) => {
          ...
          if (c.type === 'select') return <td key={String(c.key)} style={{ padding: '8px 6px' }}><MantineSelect value={(raw ?? '') as string} disabled={c.readOnly} onChange={(v) => onChange(row.id, c.key, v ?? '')} data={(c.options ?? []).map((o) => ({ value: o.value, label: o.label }))} styles={{ input: { ...inputBaseStyle }, dropdown: { background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.12)' } }} /></td>
          if (c.type === 'number') return <td key={String(c.key)} style={{ padding: '8px 6px' }}><input className="ui-input" type="number" step={c.step ?? '0.01'} value={formatNumberInput(raw)} readOnly={c.readOnly} onChange={(e) => onChange(row.id, c.key, parseNumberInput(e.target.value, Boolean(c.nullable)))} style={{ ...inputBaseStyle, backgroundColor: c.readOnly ? '#111827' : 'rgba(255,255,255,.06)' }} /></td>
          return <td key={String(c.key)} style={{ padding: '8px 6px' }}><input className="ui-input" type="text" value={(raw ?? '') as string} readOnly={c.readOnly} onChange={(e) => onChange(row.id, c.key, e.target.value)} style={{ ...inputBaseStyle, backgroundColor: c.readOnly ? '#111827' : 'rgba(255,255,255,.06)' }} /></td>
        })}<td style={{ padding: '8px 6px' }}>{renderActions && <span style={{ marginRight: 8 }}>{renderActions(row)}</span>}<button className="btn-danger-soft" onClick={() => onDelete(row.id)} style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid #7f1d1d', backgroundColor: '#2a1111', color: '#fecaca', cursor: 'pointer' }}>删除</button></td></tr>)}</tbody>
      </table>
    </div>
  )
}
export default function Admin() {
  ...
  const [settingsAutoUpdateEnabled, setSettingsAutoUpdateEnabled] = useState(true)
  const [settingsUpdateRepo, setSettingsUpdateRepo] = useState('AmBiTTion/NB-JP---IN-QUOTER')
  ...
  const [createProductDraft, setCreateProductDraft] = useState<Product | null>(null)
  ...
  const saveCreateProductModal = useCallback(() => {
    if (!createProductDraft) return
    if (isBlank(createProductDraft.name)) {
      setError('产品名称不能为空')
      return
    }
    if (isBlank(createProductDraft.pol_port_id)) {
      setError('起运港不能为空')
      return
    }
    setTables((prev) => ({ ...prev, products: [...prev.products, createProductDraft] }))
    markTableDirty('products')
    setStatus('已新增产品（请保存）')
    closeCreateProductModal()
  }, [createProductDraft, markTableDirty, closeCreateProductModal])
  ...
  const applyProductDefaults = useCallback((current: TableState) => {
    ...
    const nextPackaging = [...current.packaging_options]; const nextContainers = [...current.container_load_rules]
    ...
    return { tables: { ...current, packaging_options: nextPackaging, container_load_rules: nextContainers }, touched: [...touched] }
  }, [data?.products])
  ...
  const saveSettings = useCallback(async (options: { reload?: boolean; source?: 'manual' | 'auto' } = {}) => {
    ...
    // @ts-ignore
    const result = (await window.ipcRenderer.invoke('update-settings', { fx_rate: fx, margin_pct: margin, quote_valid_days: quoteDays, ui_theme: settingsUiTheme, auto_update_enabled: settingsAutoUpdateEnabled, update_repo: settingsUpdateRepo, money_format: { rmb_decimals: rmb, usd_decimals: usd }, pricing_formula_mode: settingsPricingFormulaMode, rounding_policy: settingsRoundingPolicy, terms_template: settingsTermsTemplate })) as { success: boolean; message?: string }
    ...
    if (!reload) {
      setData((prev) => prev ? ({ ...prev, settings: { ...prev.settings, fx_rate: fx, margin_pct: margin, quote_valid_days: quoteDays, ui_theme: settingsUiTheme, auto_update_enabled: settingsAutoUpdateEnabled, update_repo: settingsUpdateRepo, money_format: { rmb_decimals: rmb, usd_decimals: usd }, pricing_formula_mode: settingsPricingFormulaMode, rounding_policy: settingsRoundingPolicy, terms_template: settingsTermsTemplate } }) : prev)
    }
    if (reload) await loadData()
  }, [settingsFxRate, settingsMarginPct, settingsQuoteValidDays, settingsRmbDecimals, settingsUsdDecimals, settingsPricingFormulaMode, settingsRoundingPolicy, settingsUiTheme, settingsAutoUpdateEnabled, settingsUpdateRepo, settingsTermsTemplate, loadData])
  ...
  const handleCheckUpdate = useCallback(async () => {
    try {
      ...
      setError('')
      // @ts-ignore
      const result = (await window.ipcRenderer.invoke('check-for-updates')) as {
        success: boolean
        hasUpdate: boolean
        currentVersion: string
        latestVersion: string | null
      ...
      if (!result.success) setError(result.message || '更新检查失败')
    } catch (e) {
      setError(`更新检查失败: ${String(e)}`)
    } finally {
  ...
  useEffect(() => {
    ...
    if (!settingsAutoUpdateEnabled) return
    if (updateInfo || checkingUpdate) return
    void handleCheckUpdate()
  }, [activeTab, settingsAutoUpdateEnabled, updateInfo, checkingUpdate, handleCheckUpdate])
  ...
  return (
    <div className="admin-page" style={{ minHeight: '100vh', backgroundColor: '#0b0f1a', color: '#e5e7eb', padding: 20 }}>
      <h1 style={{ marginTop: 0 }}>数据维护</h1>
      <div style={{ marginBottom: 12, display: 'flex', gap: 8, alignItems: 'center' }}>
        <button className="btn-outline-neon" onClick={() => void loadData()} style={{ padding: '8px 12px', borderRadius: 6, border: '1px solid #334155', backgroundColor: '#0f172a', color: '#fff', cursor: 'pointer' }}>刷新</button>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>{TABS.map((tab) => <button className={`tab-btn ${activeTab === tab.key ? 'active' : ''}`} key={tab.key} onClick={() => setActiveTab(tab.key)} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #1f2937', backgroundColor: activeTab === tab.key ? '#1f2937' : '#0f172a', color: '#fff', cursor: 'pointer' }}>{tab.label}</button>)}</div>      {activeTab === 'settings' && (
        <div className="panel" style={sectionStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 12 }}>
            ...
            <div><label>{labelFor('pricing_formula_mode')}</label><MantineSelect mt={6} value={settingsPricingFormulaMode} onChange={(v) => { setSettingsPricingFormulaMode(v ?? 'divide'); setDirtySettings(true); setAutoSaveState('idle') }} data={[{ value: 'divide', label: 'cost/(1-margin)' }]} styles={{ input: { ...inputBaseStyle }, dropdown: { background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.12)' } }} /></div>
            <div><label>{labelFor('rounding_policy')}</label><MantineSelect mt={6} value={settingsRoundingPolicy} onChange={(v) => { setSettingsRoundingPolicy(v ?? 'ceil'); setDirtySettings(true); setAutoSaveState('idle') }} data={[{ value: 'ceil', label: '向上取整' }]} styles={{ input: { ...inputBaseStyle }, dropdown: { background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.12)' } }} /></div>
            <div>
            ...
            <div>
              ...
              <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                <input
                  type="checkbox"
                  checked={settingsAutoUpdateEnabled}
                  onChange={(e) => { setSettingsAutoUpdateEnabled(e.target.checked); setDirtySettings(true); setAutoSaveState('idle') }}
                />
                <span style={{ color: '#cbd5e1' }}>启动时检查 GitHub 新版本</span>
              </div>
            </div>
            <div>
              <label>更新仓库（owner/repo）</label>
              <input
                type="text"
                value={settingsUpdateRepo}
                onChange={(e) => { setSettingsUpdateRepo(e.target.value); setDirtySettings(true); setAutoSaveState('idle') }}
                style={{ ...inputBaseStyle, marginTop: 6, padding: 8 }}
                placeholder="AmBiTTion/NB-JP---IN-QUOTER"
              />
            </div>
            <div style={{ display: 'flex', alignItems: 'end', gap: 8 }}>
              <button
                ...
                style={{ padding: '8px 12px', borderRadius: 6, border: '1px solid #334155', backgroundColor: '#0f172a', color: '#fff', cursor: 'pointer', height: 38 }}
              ...
              {updateInfo?.hasUpdate && (
                <button
                  ...
                  onClick={() => void window.ipcRenderer.invoke('open-update-download-page', { url: updateInfo.htmlUrl })}
                  style={{ padding: '8px 12px', borderRadius: 6, border: 'none', backgroundColor: '#2563eb', color: '#fff', cursor: 'pointer', height: 38, fontWeight: 700 }}
                >
                  立即更新
                </button>
              )}
            </div>
            <div style={{ gridColumn: '1 / span 3', color: '#93c5fd', fontSize: 13 }}>
              {updateInfo
                ? `当前版本 ${updateInfo.currentVersion}，最新版本 ${updateInfo.latestVersion ?? '-'}。${updateInfo.message}`
                : '未检查更新'}
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'end' }}><button className="btn-primary" onClick={() => void saveSettings()} style={{ padding: '8px 12px', borderRadius: 6, border: 'none', backgroundColor: '#4ade80', color: '#000', cursor: 'pointer', fontWeight: 700, height: 38 }}>保存</button></div>
          </div>
        </div>
      )}
        ...
        <div className="panel" style={sectionStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="btn-outline-neon" onClick={() => { if (activeTab === 'products') openCreateProductModal(); else addRow(activeTab) }} style={{ padding: '8px 12px', borderRadius: 6, border: '1px solid #334155', backgroundColor: '#0f172a', color: '#fff', cursor: 'pointer' }}>新增</button>
              <button className="btn-primary" onClick={() => void saveTable(activeTab)} style={{ padding: '8px 12px', borderRadius: 6, border: 'none', backgroundColor: '#3b82f6', color: '#fff', cursor: 'pointer', fontWeight: 700 }}>保存</button>
            </div>
          </div>

          {activeTab === 'products' && <div>{tables.products.map((row) => <div className="subpanel" key={row.id} style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 16, marginBottom: 20, background: 'rgba(255,255,255,0.02)' }}>
            <div style={{ marginBottom: 12, color: '#e2e8f0', fontWeight: 700, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button className="btn-danger-soft" onClick={() => deleteRow('products', row.id)} style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid #7f1d1d', backgroundColor: '#2a1111', color: '#fecaca', cursor: 'pointer' }}>删除</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr 1.2fr 0.8fr 0.8fr 0.8fr 1fr', gap: 8, marginBottom: 12 }}>
              ...
              <div>
                ...
                <MantineSelect value={row.pol_port_id ?? ''} onChange={(v) => updateRow('products', row.id, 'pol_port_id', v ?? '')} data={portOptions} styles={{ input: { ...inputBaseStyle }, dropdown: { background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.12)' } }} />
              </div>
            </div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <MantineSelect
                        ...
                        onChange={(v) =>
                          updateRow('packaging_options', pack.id, 'product_id', v ?? '')
                        }
                        data={productOptions}
                        styles={{ input: { ...inputBaseStyle }, dropdown: { background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.12)' } }}
                      />
                    </div>
                    <div>
                      <div style={{ color: '#9ca3af', fontSize: 12, marginBottom: 6 }}>名称</div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <MantineSelect
                        ...
                        onChange={(v) =>
                          updateRow('packaging_options', pack.id, 'inner_pack_type', v ?? 'none')
                        }
                        data={innerPackOptions.map((option) => ({ value: option.value, label: option.label }))}
                        styles={{ input: { ...inputBaseStyle }, dropdown: { background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.12)' } }}
                      />
                    </div>
                    <div>
      ...
      {isCreateProductModalOpen && createProductDraft && (
        <div
          style={{
            position: 'fixed',
            inset: 0,
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <div style={{ fontSize: 18, fontWeight: 700, color: '#e2e8f0' }}>新增产品</div>
              <button
                className="btn-outline-neon"
                onClick={closeCreateProductModal}
                style={{ padding: '4px 10px', borderRadius: 8, border: '1px solid #334155', backgroundColor: '#0f172a', color: '#fff', cursor: 'pointer' }}
              >
                关闭
              </button>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr 1.2fr 0.8fr 0.8fr 0.8fr 1fr', gap: 8, marginBottom: 12 }}>
              <div>
                <div style={{ color: '#9ca3af', fontSize: 12, marginBottom: 6 }}>ID</div>
                <input type="text" value={createProductDraft.id} readOnly style={{ ...inputBaseStyle, backgroundColor: '#111827' }} />
              </div>
              <div>
                <div style={{ color: '#9ca3af', fontSize: 12, marginBottom: 6 }}>名称</div>
                <input type="text" value={createProductDraft.name ?? ''} onChange={(e) => setCreateProductDraft((prev) => (prev ? { ...prev, name: e.target.value } : prev))} style={inputBaseStyle} />
              </div>
              <div>
                <div style={{ color: '#9ca3af', fontSize: 12, marginBottom: 6 }}>Name (EN)</div>
                <input type="text" value={createProductDraft.name_en ?? ''} onChange={(e) => setCreateProductDraft((prev) => (prev ? { ...prev, name_en: e.target.value } : prev))} style={inputBaseStyle} />
              </div>
              <div>
                <div style={{ color: '#9ca3af', fontSize: 12, marginBottom: 6 }}>出口退税率</div>
                <input type="number" value={formatNumberInput(createProductDraft.refund_rate)} onChange={(e) => setCreateProductDraft((prev) => (prev ? { ...prev, refund_rate: Number(parseNumberInput(e.target.value, false)) } : prev))} style={inputBaseStyle} />
              </div>
              <div>
                <div style={{ color: '#9ca3af', fontSize: 12, marginBottom: 6 }}>采购增值税率</div>
                <input type="number" value={formatNumberInput(createProductDraft.purchase_vat_rate)} onChange={(e) => setCreateProductDraft((prev) => (prev ? { ...prev, purchase_vat_rate: Number(parseNumberInput(e.target.value, false)) } : prev))} style={inputBaseStyle} />
              </div>
              <div>
                <div style={{ color: '#9ca3af', fontSize: 12, marginBottom: 6 }}>开票加点</div>
                <input type="number" value={formatNumberInput(createProductDraft.invoice_tax_point)} onChange={(e) => setCreateProductDraft((prev) => (prev ? { ...prev, invoice_tax_point: Number(parseNumberInput(e.target.value, false)) } : prev))} style={inputBaseStyle} />
              </div>
              <div>
                <div style={{ color: '#9ca3af', fontSize: 12, marginBottom: 6 }}>起运港</div>
                <MantineSelect
                  value={createProductDraft.pol_port_id ?? ''}
                  onChange={(v) => setCreateProductDraft((prev) => (prev ? { ...prev, pol_port_id: v ?? '' } : prev))}
                  data={portOptions}
                  styles={{ input: { ...inputBaseStyle }, dropdown: { background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.12)' } }}
                />
              </div>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <textarea
                className="no-scroll"
                value={createProductDraft.description_en ?? ''}
                onChange={(e) => setCreateProductDraft((prev) => (prev ? { ...prev, description_en: e.target.value } : prev))}
                style={{ ...inputBaseStyle, minHeight: 96, padding: '8px 10px' }}
              />
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              <button
                className="btn-outline-neon"
                onClick={closeCreateProductModal}
                style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #334155', backgroundColor: '#0f172a', color: '#fff', cursor: 'pointer' }}
              >
                取消
              </button>
              <button
                className="btn-primary"
                onClick={saveCreateProductModal}
                style={{ padding: '8px 12px', borderRadius: 8, border: 'none', backgroundColor: '#3b82f6', color: '#fff', cursor: 'pointer', fontWeight: 700 }}
              >
                保存并关闭
              </button>
            </div>
